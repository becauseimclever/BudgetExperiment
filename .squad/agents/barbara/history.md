...
### 2026-04-13: Feature 161 Test Audit (Slice 1 Client-Side)
- **Task:** Audit whether current test suite proves Feature 161 (BudgetScope removal) is complete
- **Outcome:** Created `.squad/decisions/inbox/barbara-161-test-audit.md` decision record
- **Verdict:** ⚠️ **REJECT** — Slice 1 client tests are excellent (prove client forces Shared), but insufficient integration coverage to claim feature complete
- **Key gaps:** No proof application/API layers accept Shared-only model; no integration tests; legacy Personal data visibility unproven
- **Recommendation:** Merge Slice 1 with note that Phase 2 must include integration tests to re-prove "application behavior unchanged"
- **Status:** AUDIT COMPLETE, DECISION FILED
...
### 2026-04-13 — Feature 161 Slice 1: Hidden Scope Regression

- `src/BudgetExperiment.Client/Components/Forms/AccountForm.razor` must hard-pin `Model.Scope` to `"Shared"` in `OnParametersSet()`, not merely hide the UI toggle. Otherwise legacy edit models carrying `"Personal"` survive the hidden form path and submit stale scope semantics.
- `tests/BudgetExperiment.Client.Tests/Components/Forms/AccountFormTests.cs` already contains the right regression tripwire: `Render_PersonalScope_DefaultsToShared`. Keep that test when collapsing UI choices so hidden model state cannot silently regress.
- Targeted client validation for this slice used `dotnet test C:\ws\BudgetExperiment\tests\BudgetExperiment.Client.Tests\BudgetExperiment.Client.Tests.csproj --filter "Category!=Performance&FullyQualifiedName~AccountFormTests|FullyQualifiedName~NavMenuTests|FullyQualifiedName~SettingsPageTests|FullyQualifiedName~ScopeServiceTests|FullyQualifiedName~ScopeMessageHandlerTests" --no-restore`.
...
### 2026-04-13: Feature 161 Slice 1 Revision — Scope Leak Fix

- **Task:** Fix AccountForm scope leak in Feature 161 (BudgetScope Removal) Slice 1
- **Reason:** Lucius's slice removed visible scope UI but left a hidden path — legacy edit/create account models could still carry 'Personal' scope values
- **Solution:** Added scope coercion in AccountForm.OnParametersSet() to normalize incoming 'Personal' models to 'Shared'
- **Files Modified:** src/BudgetExperiment.Client/Components/Forms/AccountForm.razor, docs/161-budget-scope-removal.md
- **Validation:** 63 passed, 0 failed (was 62 passed, 1 failed)
- **Decision:** Merged to squad decisions.md, Feature 161 slice 1 approved
- **Status:** COMPLETE ✅
