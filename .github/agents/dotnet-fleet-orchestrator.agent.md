...
- Reject or repair misplaced audit docs: if audit findings docs are generated under `docs/*.md` in `docs/` root, relocate and consolidate them into `docs/audit/YYYY-MM-DD-<feature-slug>.md`.
- Require a post-audit sanitation check before final response to confirm no misplaced audit findings docs remain in `docs/` root.
...
6. Run post-audit sanitation checks and repair any misplaced audit findings docs in `docs/` root by relocating to `docs/audit/YYYY-MM-DD-<feature-slug>.md`.
7. Reconcile specialist outputs, resolve conflicts, and produce a unified final response.
8. Always provide a completion summary of delegation decisions and outcomes.
