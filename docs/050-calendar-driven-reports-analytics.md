...
- [ ] **Fix:** Derive currency from transaction data instead of hardcoding `"USD"` (or add a TODO with tracked issue)
- [ ] **Verify:** Scope filtering is applied by `GetByDateRangeAsync` — add explicit filter in `ReportService` if not
- [ ] Update OpenAPI documentation (tags, examples)
...
**Tasks:**
- [ ] Create `DateRangePicker.razor` component with presets ("This Month", "Last Month", "Last 7 Days", "Last 30 Days", "Custom")
- [ ] Implement start/end date inputs using native `<input type="date">` for accessibility
- [ ] Style component following existing design patterns (responsive, theme-aware)
- [ ] Write bUnit tests (preset calculation, end >= start validation, event callbacks)
- [ ] Integrate `DateRangePicker` into `MonthlyCategoriesReport.razor`
- [ ] Update page to read `?start=` and `?end=` query params, falling back to `?year=&month=` for backward compatibility
- [ ] Add `GetCategoryReportByRangeAsync` and `GetSpendingTrendsAsync` to `IBudgetApiService` + implement
- [ ] Create `BarChart.razor` component (pure SVG, grouped bars, hover tooltips, ARIA labels, responsive)
- [ ] Write bUnit tests for BarChart (empty state, single bar, grouped bars, accessibility)
- [ ] Create `MonthlyTrendsReport.razor` page at `/reports/trends`
- [ ] Add month count selector (6, 12, 24 months)
- [ ] Implement bar chart showing monthly income/spending/net over selected period
- [ ] Add category filter dropdown (optional single-category view)
- [ ] Subscribe to `ScopeService.ScopeChanged` on both new/modified pages
- [ ] Update `ReportsIndex.razor`: enable "Monthly Trends" card link, remove "Coming Soon" badge
- [ ] Add "Back to Calendar" navigation link on report pages
