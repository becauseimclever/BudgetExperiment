...
- [x] `KaizenGoal` entity created with all required fields.
- [x] EF Core configuration includes composite unique key (UserId, WeekStartDate).
- [x] Database migration creates schema with correct column types and indexes.
- [x] Create endpoint `POST /api/v1/goals/kaizen/week/{weekStart}` works.
- [x] Get endpoint `GET /api/v1/goals/kaizen/week/{weekStart}` returns goal or null.
- [x] Update endpoint `PUT /api/v1/goals/kaizen/{goalId}` allows marking achievement status.
- [x] Delete endpoint `DELETE /api/v1/goals/kaizen/{goalId}` removes goal.
- [x] Range endpoint `GET /api/v1/goals/kaizen?from=...&to=...` supports 12-week queries (for Kaizen Dashboard).
- [x] `KaizenGoalModal.razor` component renders with description, target amount, and category fields.
- [x] Modal validates description (required, max 500 chars) and target amount (>= 0 if provided).
- [x] Week summary card displays goal and status badge (✓/✗) if goal exists.
- [x] Week summary card shows "Set a goal..." CTA if no goal.
- [x] Clicking "Set a goal..." or card opens goal-setting modal.
- [x] Goal-end reminder prompt appears for weeks past their end date.
- [x] Reminder allows user to mark goal as achieved/not achieved.
- [x] Feature flag `Features:Kaizen:MicroGoals` gates all UI (hidden when disabled).
- [x] API endpoints validate authorization (users can only access their own goals).
- [x] All text fields validate char limits (Description: 500 max).
- [ ] Kaizen Dashboard report shows 12-week rolling view with goal outcomes overlaid. (Feature 137)
- [x] All existing tests pass; new tests for service, endpoints, authorization, components, and goal achievement logic.
- [x] Accessibility: labels, ARIA, keyboard navigation in modal and week summary.
