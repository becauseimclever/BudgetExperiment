...
**Result:** Users must first have spending in a category before they can budget for it — the opposite of proactive budgeting.
...
**Acceptance Criteria:**
- [x] Can create a budget goal for any active expense category
- [x] Budget goal is immediately visible on the Budget page
- [x] Category appears in monthly progress with $0 spent / $X budgeted
...
**Acceptance Criteria:**
- [x] Budget page shows all active expense categories
- [x] Categories without goals show "No Budget Set" status
- [x] Can add/edit budget goal for any listed category
...
**Acceptance Criteria:**
- [x] Category form includes optional "Monthly Budget" field
- [x] Setting a budget creates a goal for the current month
- [x] Can update budget target inline on the Categories page
...
1. **BudgetProgressService** — Fetch all active expense categories and merge with transaction/goal data
2. **BudgetSummaryDto.CategoryProgress** — Include entries with `SpentAmount = 0` and `Status = NoBudgetSet`
3. **Budget.razor** — Show "Add Budget" button for categories with no goal
...
1. **CategoryCard** — Add budget amount display and edit button
2. **CategoryForm** — Add optional "Monthly Budget" field
3. **New API endpoint** — `GET /api/v1/categories/{id}/budget?year={y}&month={m}`
...
**Tasks:**
- [x] Update `BudgetProgressService.GetMonthlySummaryAsync` to fetch all active expense categories
- [x] Merge categories without transactions/goals into `CategoryProgress` with zero spent
- [x] Write unit tests for new behavior
...
**Tasks:**
- [x] Update Budget.razor to show categories without goals
- [x] Add "Set Budget" button/action for `NoBudgetSet` categories
- [x] Reuse existing edit modal for goal creation
- [x] Ensure modal works for both create (new goal) and edit (existing goal)
...
**Tasks:**
- [x] Add optional `InitialBudget` field to `BudgetCategoryCreateDto`
- [x] Update category creation API to optionally create a budget goal
- [x] Update CategoryForm component to include budget field
...
│  │ $2,500.00  │ $1,847.32  │ $652.68    │                   │
│  └────────────┴────────────┴────────────┘                   │
│  ████████████████████░░░░░░░░░ 74%                         │
│  ✓ 4 on track  ⚠️ 1 warning  ⬚ 2 no budget                 │
├─────────────────────────────────────────────────────────────┤
│  By Category                                                │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ 🍔 Groceries            $450 / $500    90% ⚠️  [Edit]│   │
│  │ 🚗 Transportation       $120 / $200    60% ✓   [Edit]│   │
│  │ 🎮 Entertainment        $0 / $100       0% ✓   [Edit]│   │
│  │ 👕 Clothing             $0 / -        N/A      [Set] │   │ ← NEW
│  │ 🎁 Gifts                $0 / -        N/A      [Set] │   │ ← NEW
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
- [ ] Create new expense category → appears on Budget page with "Set Budget"
- [ ] Set budget for category with no transactions → shows progress bar at 0%
- [ ] Delete budget goal → category shows "No Budget Set"
- [ ] Navigate between months → correct goals displayed

---

## Dependencies

- Feature 021 (Budget Categories & Goals) — **COMPLETED**
