...
public sealed class SuggestionsControllerTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/v1/suggestions?type=NewRule");
    ...
    {
        // Act
        var response = await _client.GetAsync($"/api/v1/suggestions/{Guid.NewGuid()}");
    ...
    public async Task Generate_Returns_ValidResponse()
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync("/api/v1/suggestions/generate", request);
    ...
    {
        // Act
        var response = await _client.PostAsync($"/api/v1/suggestions/{Guid.NewGuid()}/accept", null);
    ...
    public async Task Dismiss_Returns_404_WhenNotFound()
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync($"/api/v1/suggestions/{Guid.NewGuid()}/dismiss", request);
    ...
    public async Task Feedback_Returns_404_WhenNotFound()
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync($"/api/v1/suggestions/{Guid.NewGuid()}/feedback", request);
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/v1/suggestions/metrics");
