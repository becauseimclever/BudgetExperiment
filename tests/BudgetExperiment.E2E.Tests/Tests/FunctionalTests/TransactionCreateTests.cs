...
public class TransactionCreateTests
{
    ...
    public async Task CreateTransaction_ShouldAppearInAccountTransactionsList()
    {
        ...
        var addModal = page.Locator(".modal-dialog", new()
        {
        ...
        await addModal.GetByRole(AriaRole.Button, new()
        {
            Name = "Save",
        }).ClickAsync();

        // Wait for modal to close (confirms save succeeded)
        await Expect(addModal).Not.ToBeVisibleAsync(new()
        {
            Timeout = 15000,
        });
        ...
        var transactionRow = page.Locator("tr", new()
        {
            HasText = description,
        });
        await Expect(transactionRow).ToBeVisibleAsync(new()
        {
            Timeout = 10000,
        });
    ...
    public async Task CreateTransaction_ShouldAppearOnCalendar()
    {
        ...
        await Expect(page.Locator("tr", new()
        {
            HasText = description,
        ...
        var accountOption = accountSelect.Locator("option", new()
        {
            HasText = accountName,
        }).First;
