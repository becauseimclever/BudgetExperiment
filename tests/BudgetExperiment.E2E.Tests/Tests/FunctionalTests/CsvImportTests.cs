...
public class CsvImportTests
{
    ...
    public async Task Import_ShouldUploadMapPreviewAndCreateTransactions()
    {
        ...
        await Expect(page.GetByText("Parsed")).ToBeVisibleAsync(new()
        {
            Timeout = 20000,
        });
        ...
        var previewButton = page.GetByRole(AriaRole.Button, new()
        {
            Name = "Preview Import",
        });
        ...
        var importButton = page.GetByRole(AriaRole.Button, new()
        {
        ...
        await Expect(page.GetByText("Import Successful!")).ToBeVisibleAsync(new()
        {
            Timeout = 20000,
        });
    ...
    {
        var dateRow = page.Locator(".mapping-row", new()
        {
            HasText = "Date",
        }).First;
        var descriptionRow = page.Locator(".mapping-row", new()
        {
            HasText = "Description",
        }).First;
        var amountRow = page.Locator(".mapping-row", new()
        {
            HasText = "Amount",
        }).First;
