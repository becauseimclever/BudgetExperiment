// <copyright file="GroupedBarChart.razor.cs" company="BecauseImClever">
...
/// <summary>
/// Pure SVG grouped bar chart component for rendering multi-series bar data.
/// </summary>
public partial class GroupedBarChart
{
    /// <summary>
    ...
    /// <summary>
    /// Gets or sets a value indicating whether to show values on bars.
    /// </summary>
    [Parameter]
    ...
    /// </summary>
    [Parameter]
    public string AriaLabel { get; set; } = "Grouped bar chart";
    ...
    private decimal ComputedMaxValue
    {
        get
        {
            var values = Data.SelectMany(group => group.Values.Values).ToList();
            if (values.Count == 0)
            {
                return 100m;
            }
    ...
    private IReadOnlyList<GroupInfo> BuildGroups()
    {
        ...
        var seriesCount = Math.Max(seriesIds.Count, 1);
        var barSpacing = 4d;
        var barWidth = Math.Max((usableWidth - ((seriesCount - 1) * barSpacing)) / seriesCount, 2);
        ...
        for (var gi = 0; gi < Data.Count; gi++)
        {
            ...
            for (var si = 0; si < seriesIds.Count; si++)
            {
                var seriesId = seriesIds[si];
                ...
                var x = groupX + (si * (barWidth + barSpacing));
                var y = MarginTop + ChartAreaHeight - clampedHeight;
    ...
    {
        var ratio = ComputedMaxValue > 0 ? (double)(value / ComputedMaxValue) : 0;
    ...
    private void HandleBarHover(BarRectInfo rect)
    {
        HoveredBar = new HoveredBarInfo
        {
            GroupLabel = rect.GroupLabel,
            SeriesLabel = rect.SeriesLabel,
            FormattedValue = rect.FormattedValue,
        };
    }
    ...
    private void HandleBarClick(BarRectInfo rect)
    {
        ...
        {
            OnBarClick.InvokeAsync((rect.Group, rect.SeriesId));
        }
    }
    ...
    private sealed class HoveredBarInfo
    {
        public string GroupLabel { get; set; } = string.Empty;
